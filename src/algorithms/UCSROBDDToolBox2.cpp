...
namespace UCSROBDDToolBox2
{
			  ...
			  unsigned int * max_graph_size)
	{
		...
		while (Stack.size () > 0)
		{
			...
			do
			{
				X = UCSROBDDToolBox2::select_an_unvisited_adjacent (&Graph, R, Y, &i);
				...
				else
				{
					...
					Graph.insert (pair<string, UCSROBDDToolBox2::Node *> (X->vertex->print_subset (), X));
					...
					if (X->vertex->cost > Y->vertex->cost)
					{
						...
						{
							UCSROBDDToolBox2::update_upper_restriction (R, X->vertex);
							X->upper_flag->set_empty_subset ();
							Y->upper_flag->remove_element (i);
							UCSROBDDToolBox2::prune_upper_elements (&Graph, &Stack, X);
						}
						else  // Proposition 3.1
						{
							UCSROBDDToolBox2::update_lower_restriction (R, X->vertex);
							X->lower_flag->set_empty_subset ();
							Y->lower_flag->remove_element (i);
							UCSROBDDToolBox2::prune_lower_elements (&Graph, &Stack, X);
						}
					}
					else if (X->vertex->cost < Y->vertex->cost)
					{
						...
						{
							UCSROBDDToolBox2::update_lower_restriction (R, Y->vertex);
							Y->lower_flag->set_empty_subset ();
							X->lower_flag->remove_element (i);
							UCSROBDDToolBox2::prune_lower_elements (&Graph, &Stack, Y);
						}
						else  // Proposition 3.2
						{
							UCSROBDDToolBox2::update_upper_restriction (R, Y->vertex);
							Y->upper_flag->set_empty_subset ();
							X->upper_flag->remove_element (i);
							UCSROBDDToolBox2::prune_upper_elements (&Graph, &Stack, Y);
						}
					}
		...
		for (it = Graph.begin (); it != Graph.end (); )
		{
			if (it->second->upper_flag->is_empty ())
				UCSROBDDToolBox2::update_lower_restriction (R, it->second->vertex);
			else if (it->second->lower_flag->is_empty ())
				UCSROBDDToolBox2::update_upper_restriction (R, it->second->vertex);
	...
	UCSROBDDToolBox2::Node * create_node (ElementSubset * X)
	{
		Node * N = new Node;
