...
namespace UCSOBDDToolBox
{
	...
	void update_upper_restriction (OBDD * R, ElementSubset * A)
	{
		R->add_interval (A, true);
	}


	void DFS (Node * M, Collection * L, OBDD * R, CostFunction * c,
			  unsigned int * max_graph_size)
	{
		...
		while (Stack.size () > 0)
		{
			...
			do
			{
				X = UCSOBDDToolBox::select_an_unvisited_adjacent (&Graph, R, Y, &i);
				...
				else
				{
					...
					Graph.insert (pair<string, UCSOBDDToolBox::Node *> (X->vertex->print_subset (), X));
					...
					if (X->vertex->cost > Y->vertex->cost)
					{
						...
						{
							UCSOBDDToolBox::update_upper_restriction (R, X->vertex);
							X->upper_flag->set_empty_subset ();
							Y->upper_flag->remove_element (i);
							UCSOBDDToolBox::prune_upper_elements (&Graph, &Stack, X);
						}
						else  // Proposition 3.1
						{
							UCSOBDDToolBox::update_lower_restriction (R, X->vertex);
							X->lower_flag->set_empty_subset ();
							Y->lower_flag->remove_element (i);
							UCSOBDDToolBox::prune_lower_elements (&Graph, &Stack, X);
						}
					}
					else if (X->vertex->cost < Y->vertex->cost)
					{
						...
						{
							UCSOBDDToolBox::update_lower_restriction (R, Y->vertex);
							Y->lower_flag->set_empty_subset ();
							X->lower_flag->remove_element (i);
							UCSOBDDToolBox::prune_lower_elements (&Graph, &Stack, Y);
						}
						else  // Proposition 3.2
						{
							UCSOBDDToolBox::update_upper_restriction (R, Y->vertex);
							Y->upper_flag->set_empty_subset ();
							X->upper_flag->remove_element (i);
							UCSOBDDToolBox::prune_upper_elements (&Graph, &Stack, Y);
						}
					}
		...
		for (it = Graph.begin (); it != Graph.end (); )
		{
			if (it->second->upper_flag->is_empty ())
				UCSOBDDToolBox::update_lower_restriction (R, it->second->vertex);
			else if (it->second->lower_flag->is_empty ())
				UCSOBDDToolBox::update_upper_restriction (R, it->second->vertex);
	...
	Node * select_an_unvisited_adjacent (map<string, Node *> * Graph, OBDD * R, Node * Y, unsigned int * i)
	{
	...
	UCSOBDDToolBox::Node * create_node (ElementSubset * X)
	{
		Node * N = new Node;
