//
// SFFS.cpp -- implementation of the class "SFFS" ("Sequential Forward Floating Selection").
//
...
void SFFS::get_minima_list (unsigned int max_size_of_minima_list)
{
	...
	while ( (best < set->get_set_cardinality ()) && (! cost_function->has_reached_threshold ()) )
	{
		...
		//
		if (J[k-1] <= Y.cost)  // J[k-1] == c(X - {best})
