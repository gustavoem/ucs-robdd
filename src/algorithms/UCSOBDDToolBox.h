#ifndef UCSOBDDTOOLBOX_H_
#define UCSOBDDTOOLBOX_H_
...
namespace UCSOBDDToolBox
{
	...
	//
	ElementSubset * get_maximal_subset (OBDD *, ElementSet *);
	...
	//
	ElementSubset * get_minimal_subset (OBDD *, ElementSet *);


	// Receives a subset A, a collection of restrictions and add A into the latter.
	//
	void update_lower_restriction(OBDD *, ElementSubset *);


	// Receives a subset A, a collection of restrictions and add A into the latter.
	//
	void update_upper_restriction(OBDD *, ElementSubset *);
	...
	// DFS subroutine
	//
	void DFS (Node *, Collection *, OBDD *, CostFunction *, unsigned int *);
	...
	//
	Node * select_an_unvisited_adjacent (map<string, Node *> *, OBDD *, Node *, unsigned int *);
