//
// SFS.cpp -- implementation of the class "SFS" ("Sequential Forward Selection").
//
...
void SFS::find_minima_list (unsigned int max_size_of_minima_list)
{
	...
	// if k = |S| then there is no more element to add into X in order to decrease the value of c(X)
	//
	}
	while ( (X.get_subset_cardinality () < set->get_set_cardinality ()) && (k < set->get_set_cardinality ()));
