...
{
	ElementSubset * get_minimal_subset (Collection * RL, Collection * RU, ElementSet * a_set)
	{
	...
	ElementSubset * get_maximal_subset (Collection * RL, Collection * RU, ElementSet * a_set)
	{
		ElementSubset * A, B ("", a_set);
		unsigned int i;
	...
	void DFS (Node * M, Collection * L, Collection * RL, Collection * RU, CostFunction * c,
	          unsigned int * max_graph_size)
	{
		...
		while (Stack.size () > 0)
		{
			...
			do
			{
				...
				else
				{
					...
					Graph.insert (pair<string, UCurveToolBox::Node *> (X->vertex->print_subset (), X));
			...
			//
			if ((Y->lower_flag->is_empty ()) && (! RL->lower_covers(Y->vertex)) )
			{
				UCurveToolBox::update_lower_restriction (RL, Y->vertex);
				UCurveToolBox::prune_lower_elements (&Graph, &Stack, Y);
			}
			...
			//
			if ((Y->upper_flag->is_empty ()) && (! RU->upper_covers(Y->vertex)) )
			{
				UCurveToolBox::update_upper_restriction (RU, Y->vertex);
				UCurveToolBox::prune_upper_elements (&Graph, &Stack, Y);
			}
		...
		for (it = Graph.begin (); it != Graph.end (); it++)
		{
			if (it->second->upper_flag->is_empty ())
				UCurveToolBox::update_lower_restriction (RL, it->second->vertex);
			else if (it->second->lower_flag->is_empty ())
				UCurveToolBox::update_upper_restriction (RU, it->second->vertex);
	...
	Node * select_an_unvisited_adjacent
	 (map<string, Node *> * Graph, Collection * RL, Collection * RU, Node * Y, unsigned int * i)
	{
