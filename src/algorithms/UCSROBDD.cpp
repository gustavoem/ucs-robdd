...
void UCSROBDD::get_minima_list (unsigned int max_size_of_minima_list)
{
	...
	do
	{
		...
		if (direction == 0)
		{
			//-> X = UCSROBDDToolBox::get_minimal_subset (lower_restriction, upper_restriction, set);
			if (X != NULL)
			{
		...
		{
			//->X = UCSROBDDToolBox::get_maximal_subset (lower_restriction, upper_restriction, set);
			if (X != NULL)
			{
