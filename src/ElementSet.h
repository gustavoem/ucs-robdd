...
public:
    ...
    // Constructor: a set with elements extracted from a DAT file with
    // n elements.
    //
    ElementSet (string, unsigned int);


    // Constructor: a set with n random-valued elements with integers
    // between [0, range].
    //
    ...
    // Returns a pointer to the element at the "index" position. If
    // there is no element at such position, it returns NULL.
    //
    Element * get_element (unsigned int);


    // Given an Element * Y, returns an unsigned integer x such that
    // list_of_elements[x] is the same as Y.
    //
    ...
    // Permute the elements of the set. Useful to obtain different 
    // running times in solvers such as SFFS and BranchAndBound.
    //
    void permute_set ();


    // Prints in the STDOUT all the elements of this set and their
    // values.
    //
