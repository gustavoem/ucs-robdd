//
// MeanAbsSum.cpp -- implementation of the class "MeanAbsSum".
//
...
#include "MeanAbsSum.h"


MeanAbsSum::MeanAbsSum (ElementSet * a_set)
{
	unsigned int i, j;
	Element * elem;
	...
	if (a_set->get_set_cardinality () == 0)
		return;
	...
	for (i = 0; i < set->get_set_cardinality (); i++)
	{
		...
		for (j = 0; j < elem->get_number_of_values (); j++)
			t += elem->get_element_value (j);
	}

	t = (int) t / (set->get_set_cardinality () * set->get_element (0)->get_number_of_values ());
}


MeanAbsSum::~MeanAbsSum ()
{
	// TODO
}


float MeanAbsSum::cost (ElementSubset * X)
{
	timeval begin, end;
