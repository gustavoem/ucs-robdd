...
namespace CollectionTest {
	...
	bool it_should_remove_lower_covered_subsets_in_a_collection ()
	{
		...
		L.add_subset (&P);
		L.add_subset (&Q);
		if ((L.remove_lower_covered_subsets (&X) == 4)  && // should remove O,P,Q,R and keep Y,Z
		    ( L.has_subset (&Y)) &&
		    (!L.has_subset (&X)) &&
		    ( L.has_subset (&Z)) &&
		    (!L.has_subset (&O)) &&
	...
	bool it_should_remove_upper_covered_subsets_in_a_collection ()
	{
		...
		L.add_subset (&P);
		L.add_subset (&Q);
		if ((L.remove_upper_covered_subsets (&X) == 4)  && // should remove O,P,Q,R and keep Y,Z
			( L.has_subset (&Y)) &&
		    (!L.has_subset (&X)) &&
		    ( L.has_subset (&Z)) &&
		    (!L.has_subset (&O)) &&
	...
	bool the_evaluated_subset_should_not_be_deleted ()
	{
		...
		if (!L.lower_covers (&X) && (X.get_set_that_contains_this_subset () == &S))
			return true;
